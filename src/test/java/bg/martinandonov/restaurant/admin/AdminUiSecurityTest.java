...
@WebMvcTest(controllers = AdminUiForwardController.class)
...
class AdminUiSecurityTest {
	...
	@Test
	@WithAnonymousUser
	void anonymousAdminDeniedOrRedirectedToLogin() throws Exception {
		MvcResult result = mockMvc.perform(get("/admin")).andReturn();
		int status = result.getResponse().getStatus();
		org.assertj.core.api.Assertions.assertThat(status).isIn(302, 401);
	}
	...
	@Test
	@WithMockUser(roles = "ADMIN")
	void adminCanOpenAdminSlash() throws Exception {
		mockMvc.perform(get("/admin/"))
				.andExpect(status().isOk())
				.andExpect(forwardedUrl("/admin/index.html"));
	}
